type FooterOutputs = SharedGlobalOutputs & {
	footer: HTMLElement;
};
...
(() => {
	...
	const currentFooterLinkId = (() => {
		if (currentSegment === '') return 'footer-home';
		if (currentSegment === 'about') return 'footer-about';
		return null;
	})();
	...
	if (typeof globalElements == 'undefined') {
		...
		const footerOutputs = globalElements.output as FooterOutputs;
		footerOutputs.footer.innerHTML = content;
	}

	const currentPageLink = currentFooterLinkId
		? document.getElementById(currentFooterLinkId) as HTMLAnchorElement | null
		: null;
	if (currentPageLink) {
