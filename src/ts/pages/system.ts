type SystemWindow = typeof window & {
	startupFunctions?: () => void;
	locationSentence?: () => void;
	resetExternal?: () => void;
	generateGalleryArray?: () => void;
	galleryExplanationExternal?: () => string;
	addResourceInput?: (element: HTMLElement, sectionTarget: number) => void;
	removeResource?: (resourceID: string) => void;
	addResource?: (element?: HTMLInputElement | HTMLButtonElement | null) => void;
	biomeLinks?: (element: HTMLSelectElement) => void;
	infestedBiomeLinks?: (dest: string, bool: boolean | void) => void;
	moonSwitch?: (element: HTMLInputElement) => void;
	expandDescriptor?: (element: HTMLInputElement, planetClass?: string | null) => void;
	merchantUpgrades?: (group?: string | null) => void;
	tradeables?: () => void;
	enableTradeableAdd?: () => void;
	regionLong?: () => void;
	expectedHubTagSentence?: () => void;
	spaceStationSection?: () => void;
	autoBH?: () => void;
	autoPirate?: (element: HTMLSelectElement) => void;
	combineEconConf?: () => void;
	addTemplate?: (element?: HTMLInputElement | null) => void;
	civCatalog?: () => void;
	planetInputs?: () => void;
};

type SystemPageData = SharedPageData & {
	region: string;
	civShort: string;
	planet: string;
	moon: string;
...
type SystemInputs = SharedGlobalInputs & {
	terminalInputs: HTMLElement;
	waterInput: HTMLElement;
	colorInput: HTMLInputElement;
	conflictInput: HTMLSelectElement;
	wealthInput: HTMLSelectElement;
	economyInput: HTMLSelectElement;
};

type SystemOutputs = SharedGlobalOutputs & {
	planets: HTMLElement;
...
const systemWindow = window as SystemWindow;
const systemPageData = pageData as SystemPageData;
const systemInputs = globalElements.input as SystemInputs;
const systemOutputs = globalElements.output as SystemOutputs;

const systemStartupFunctions = () => {
...
systemWindow.startupFunctions = systemStartupFunctions;
...
const systemLocationSentence = () => {
	const region = systemPageData.region;
	const civ = systemPageData.civShort;
	const HubNr = regNr(region);
	const galaxy = HubGal(civ);
	const output = `Located in the [[${region}]] [[region]]${HubNr} of the ${galaxy}.`;
	wikiCode(output, 'loc');
};
systemWindow.locationSentence = systemLocationSentence;

const systemPlanetInputs = () => {
	const inputTarget = systemInputs.waterInput.parentElement?.previousElementSibling;
	const outputTarget = systemOutputs.planets;
	const planetNr = systemPageData.planet;
	const moonNr = systemPageData.moon;
	const bodies = clamp(parseInt(planetNr) + parseInt(moonNr), 2, 6);
	if (isNaN(bodies) || !inputTarget) return;
	const planetInputTarget = inputTarget as HTMLElement;
	...
	function diff() {
		const planetIDs = new Set<string | undefined>();
		const planets = (() => {
			const elements = document.querySelectorAll<HTMLElement>('[data-planet]');
	...
	function addPlanet(i: number) {
		...
		planetInputTarget.insertAdjacentHTML('beforebegin', inputTemplate);
		outputTarget.insertAdjacentHTML('beforeend', planetTemplate);

		const defaultValue = document.querySelectorAll<HTMLInputElement>(`[data-planet="planet${i}"] [data-default]`);
		for (const input of defaultValue) {
			assignFunction(input, 'assignDefaultValue(this)');
			assignDefaultValue(input);
		}
		const auto = document.querySelectorAll<Element>(`[data-planet="planet${i}"] [data-dest]`);
		for (const input of auto) {
			assignFunction(input, 'wikiCode(this)');
		}
		const noauto = document.querySelectorAll<Element>(`[data-planet="planet${i}"] [data-dest-noauto]`);
		for (const input of noauto) {
			assignFunction(input, 'storeData(this)');
		}
		const lists = document.querySelectorAll<HTMLInputElement | HTMLSelectElement>(`[data-planet="planet${i}"] [list]`);
		for (const input of lists) {
			assignFunction(input, 'forceDatalist(this)', 'onchange');
		}
		const resourceButton = document.getElementById(`addResourceButton${i}`) as HTMLElement | null;
		for (let j = 0; j < 3; j++) {
			if (resourceButton) addResourceInput(resourceButton, i);
		}

		const resourceOutputs: { output: Record<string, string> } = { output: {} };
		const outputs = document.querySelectorAll<HTMLOutputElement>(`#body${i} output`);
		...
		const biomeInput = document.getElementById(`biome_input${i}`) as HTMLSelectElement | null;
		if (biomeInput) biomeLinks(biomeInput);
		addAllTooltips();
	}
};
systemWindow.planetInputs = systemPlanetInputs;

function addResourceInput(element: HTMLElement, sectionTarget: number) {
	...
	const resourceElements: { input: Record<string, string> } = { input: {} };
...
systemWindow.addResourceInput = addResourceInput;

function getResourceInputSections(sectionTarget: number) {
	return document.querySelectorAll<HTMLElement>(`[data-resource][data-planet="planet${sectionTarget}"]`);
}

function getResourceInputSectionCount(inputs: NodeListOf<HTMLElement>) {
	const sections = new Set<string | undefined>();
...
function removeResource(resourceID: string) {
	...
	const addButton = document.getElementById(`addResourceButton${sectionTarget}`) as HTMLButtonElement | null;
	if (addButton) addButton.disabled = false;

	const resourceInputSections = getResourceInputSections(parseInt(sectionTarget, 10));
	const resourceInputSectionCount = getResourceInputSectionCount(resourceInputSections);
...
systemWindow.removeResource = removeResource;

function moonSwitch(element: HTMLInputElement) {
	const descriptorDropdown = document.getElementById(element.dataset.destNoauto as string) as HTMLInputElement | null;
	const i = extractNumber(element.id);
	const planetClass = element.checked ? 'Moon' : 'Planet';
	const planetClassOutput = document.getElementById('planetClass' + i);
	if (planetClassOutput) planetClassOutput.innerText = planetClass;
	if (descriptorDropdown) expandDescriptor(descriptorDropdown, planetClass);
}
systemWindow.moonSwitch = moonSwitch;

function expandDescriptor(element: HTMLInputElement, planetClass: string | null = null) {
	const i = extractNumber(element.id);
	if (!planetClass) {
		const moonInput = document.getElementById('moon_input' + i) as HTMLInputElement | null;
		planetClass = moonInput?.checked ? 'Moon' : 'Planet';
	}
	...
	const isInfested = autoInfested(element);
	infestedBiomeLinks('infested' + i, isInfested);
}
systemWindow.expandDescriptor = expandDescriptor;

function merchantUpgrades(group: string | null = null) {
	const checkboxes = document.querySelectorAll<HTMLElement>('[data-dest-checkbox-group]');
	if (group) {
	...
	for (const checkbox of checkboxes) {
		if (!(checkbox as HTMLInputElement).onchange) assignFunction(checkbox, 'merchantUpgrades(this.dataset.destCheckboxGroup)');
	...
	function getCheckedBoxes(group: string) {
		const checkboxes = document.querySelectorAll<HTMLInputElement>(`[data-dest-checkbox-group="${group}"]`);
		...
		for (let i = 1; i <= checked.length; i++) {
			code.push(`| ${parm}${i}=${checked[i - 1]}`);
		}
		wikiCode(code.join('\n'), group);
			const groupOutput = globalElements.output[group];
			if (!groupOutput) return;
			const wrapper = groupOutput.closest('#scrapDealer') as HTMLElement | null;
			if (!wrapper) return;
			wrapper.style.display = code.length === 0 ? 'none' : '';
	}
}
systemWindow.merchantUpgrades = merchantUpgrades;

function tradeables() {
	const inputSection = systemInputs.terminalInputs;
	const outputSection = systemOutputs.tradeTerminal;
	...
	const inputs = document.querySelectorAll<Element>(`[data-tradeable="section${childIndex}"] input[data-dest]`);
	for (const input of inputs) {
		assignFunction(input, 'wikiCode(this)');
	}
	const noautoInputs = document.querySelectorAll<Element>(`[data-tradeable="section${childIndex}"] input[data-dest-noauto]`);
	for (const input of noautoInputs) {
		assignFunction(input, 'storeData(this); numberStats(this)');
	}

	const tradeableInputSections = document.querySelectorAll<HTMLElement>('[data-tradeable]');
	const tradeableInputSectionCount = (() => {
		const sections = new Set<string | undefined>();
		for (const tradeableInputSection of tradeableInputSections) {
			sections.add(tradeableInputSection.dataset.tradeable);
		}
		return sections.size;
	})();

	const button = inputSection.querySelector('button') as HTMLButtonElement | null;
	if (button && tradeableInputSectionCount + 1 > 5) {
		button.disabled = true;
	}
}
systemWindow.tradeables = tradeables;
...
systemWindow.enableTradeableAdd = enableTradeableAdd;

const systemResetExternal = () => {
	const sections = document.querySelectorAll('[data-tradeable], [data-planet]');
	removeSection(sections);
};
systemWindow.resetExternal = systemResetExternal;

function regionLong() {
	const region = systemPageData.region;
	const output = region.split(' ').length === 1 ? region + ' region' : region;
	systemOutputs.regionLong.innerText = output;
}
systemWindow.regionLong = regionLong;

const systemAddResource = (element: HTMLInputElement | HTMLButtonElement | null = null) => {
	...
	const linkedResources = sortObj(structuredClone(resources), true) as Record<string, Record<string, string>>;
...
systemWindow.addResource = systemAddResource as (element?: HTMLInputElement | HTMLButtonElement | null) => void;
...
function biomeLinks(element: HTMLSelectElement) {
	...
	const linkedBiomes = sortObj(structuredClone(biomes), true) as Record<string, string>;
	...
	for (const key in linkedBiomes) {
		const outputElement = globalElements.output[key];
		if (outputElement) outputElement.innerText = linkedBiomes[key];
	}
}
systemWindow.biomeLinks = biomeLinks;

function infestedBiomeLinks(dest: string, bool: boolean | void) {
	const infestedBiomes = (links.infestedBiomes ??= {});
	infestedBiomes[dest] = bool ?? false;

	let infestedBiomeLink = false;
	const linkedBiomes = sortObj(structuredClone(infestedBiomes), true) as Record<string, boolean | string>;
	for (const planetName in linkedBiomes) {
	...
	for (const key in linkedBiomes) {
		const outputElement = globalElements.output[key];
		if (outputElement) outputElement.innerText = linkedBiomes[key] as string;
	}
}
systemWindow.infestedBiomeLinks = infestedBiomeLinks;

function expectedHubTagSentence() {
	const outputElement = systemOutputs.expectedHubTag;
	const systemName = systemPageData.name;
	const region = systemPageData.region;
	const glyphs = systemPageData.portalglyphs;
...
systemWindow.expectedHubTagSentence = expectedHubTagSentence;
...
function spaceStationSection() {
	...
	const sectionDefinition: Record<string, string[]> = {
		...
		const faction = (() => {
			const input = systemPageData.faction;
			if (input === 'Uncharted') return 'uncharted';
			if (input.includes('Abandoned')) return 'abandoned';
			if (systemPageData.wealth.includes('Black Market')) return 'pirate';
			return 'normal';
		})();

	const stationSections = document.querySelectorAll<HTMLElement>('[data-station]');
	for (let i = 0; i < stationSections.length; i++) {
		...
		if (sectionDefinition[faction].includes(sectionName)) {
			if ((sectionName === 'merchant' || sectionName === 'scrapDealer') && defaultDisplay(section)) {
				...
				if (section.id === 'scrapDealer' && !systemPageData.SDMerchant) {
	...
	const noteSection = document.querySelector('[data-station="note"]') as HTMLElement | null;
	if (noteSection) noteSection.innerText = notes[faction];

	function defaultDisplay(section: HTMLElement) {
		const button = section.querySelector('button') as HTMLElement | null;
		if (!button) return;
		const buttonId = button.dataset.buttonId;
		if (!buttonId) return;
		const displayId = 'display' + buttonId;
		return button.dataset[displayId];
	}
}
systemWindow.spaceStationSection = spaceStationSection;

function autoBH() {
	const glyphs = systemPageData.portalglyphs;
	const colorInput = systemInputs.colorInput;
...
systemWindow.autoBH = autoBH;

function autoPirate(element: HTMLSelectElement) {
	const value = element.value;
	if (!value.includes('Black Market') && !value.includes('Pirate Controlled')) return;
	const conflict = systemInputs.conflictInput;
	const wealth = systemInputs.wealthInput;
...
systemWindow.autoPirate = autoPirate;

function combineEconConf() {
	const faction = systemPageData.faction;
	const wealth = systemInputs.wealthInput;
	const economy = systemInputs.economyInput;
	const conflict = systemInputs.conflictInput;
...
systemWindow.combineEconConf = combineEconConf;

function addTemplate(element: HTMLInputElement | null = null) {
	if (!element) {
		const checkboxes = document.getElementsByName('systemExtras');
		for (const checkbox of checkboxes) {
			addTemplate(checkbox as HTMLInputElement);
		}
		return;
	}

	const outputElement = globalElements.output[element.value] as HTMLElement;
	outputElement.style.display = element.checked ? '' : 'none';
}
systemWindow.addTemplate = addTemplate;

function civCatalog() {
	const civ = shortenGHub(systemPageData.civShort);
	wikiCode(civ, 'civShorter');
}
systemWindow.civCatalog = civCatalog;

const systemGenerateGalleryArray = () => {
	const array = [
		'',
	...
	if (systemPageData.faction === 'Uncharted' || systemPageData.faction.includes('Abandoned')) {
		array.pop();
	}

	systemPageData.galleryArray = array;
};
systemWindow.generateGalleryArray = systemGenerateGalleryArray;

const systemGalleryExplanationExternal = () => {
...
systemWindow.galleryExplanationExternal = systemGalleryExplanationExternal;
