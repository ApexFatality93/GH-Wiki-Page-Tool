type ShipData = Record<string, {
	cost: Record<string, string>;
	slots: Record<string, string>;
	techslots: Record<string, string>;
	subtypes: string[] | Record<string, string[]>;
	secParts: string[];
	accessories: string[];
	miscParts: string[];
...
type StarshipPageData = SharedPageData & {
	type: string;
	subtype: string;
	inventory: string;
	economy: string;
...
type StarshipInputs = SharedGlobalInputs & {
	typeInput: HTMLInputElement;
	subtypeInput: HTMLSelectElement;
	inventoryInput: HTMLSelectElement;
	appearanceInput: HTMLInputElement;
	mainColourInput: HTMLInputElement;
	secColourInput: HTMLInputElement;
	secPartsInput: HTMLSelectElement;
	accessoriesInput: HTMLSelectElement;
	miscPartsInput: HTMLSelectElement;
};

type StarshipOutputs = SharedGlobalOutputs & {
	sChance: HTMLElement;
	stats: HTMLElement;
	cost: HTMLElement;
	slots: HTMLElement;
...
type StarshipWindow = typeof window & {
	startupFunctions?: () => void;
	generateGalleryArray?: () => void;
	galleryExplanationExternal?: () => string;
};

const starshipWindow = window as StarshipWindow;
const starshipPageData = pageData as StarshipPageData;
const starshipInputs = globalElements.input as StarshipInputs;
const starshipOutputs = globalElements.output as StarshipOutputs;
...
function getShipData(): ShipData {
	const defaultSections: Record<string, [string, string?]> = {
		subtypeInput: ['show'],
		exoticInput: ['hide', ''],
		pilotInput: ['hide', ''],
		inventoryInput: ['show'],
...
function showHideStarshipSelects() {
	const shipData = getShipData();
	const showState = { show: '', hide: 'none' } as const;
	const type = starshipPageData.type;
	invDropdown();

	for (const input of Object.keys(shipData[type].sections)) {
		const data = shipData[type].sections[input];
		const inputElement = globalElements.input[input] as HTMLInputElement | HTMLSelectElement;
		hideInput(inputElement, showState[data[0] as 'show' | 'hide']);
		if (data.length > 1) {
			inputElement.value = data[1] as string;
		} else if (inputElement.tagName.toLowerCase() === 'select') {
			inputElement.value ||= (inputElement.querySelector('option') as HTMLOptionElement | null)?.value ?? '';
		}
...
function invDropdown() {
	...
	const shipData = getShipData();
	if (type === 'Hauler') {
		setDropdownOptions(inventory, (shipData.Hauler.subtypes as Record<string, string[]>)[subtype]);
		if ((shipData.Hauler.subtypes as Record<string, string[]>)[subtype].length === 1) {
			inventory.value = (shipData.Hauler.subtypes as Record<string, string[]>)[subtype][0];
...
function calcInv() {
	...
	switch (type) {
		case 'Freighter':
			inventory = (shipData[type].subtypes as string[]).indexOf(subtype) > 5 ? 'Small' : 'Large';
			break;
		case 'Shuttle':
			inventory = (shipData[type].subtypes as string[]).indexOf(subtype) > 1 ? 'Medium' : 'Small';
			break;
	}
...
function shipType() {
	if (starshipPageData.type === 'Freighter') return 'freighter';
	if (starshipPageData.type === 'Derelict') return 'derelict';
	return 'starship';
}
...
function starshipLoc() {
	...
	function livingShipSpawn() {
		...
		const celestialBody = planetMoonSentence() as string;
...
function appearanceSentence() {
	...
	const addParts = (() => {
		...
		for (let i = 0; i < usedParts.length; i++) {
			let prefix = '';
			if (i !== 0 && i !== usedParts.length - 1) prefix = ', ';
			else if (i === usedParts.length - 1) prefix = ' and ';
