"use strict";
const multitoolWindow = window;
const multitoolPageData = pageData;
const multitoolInputs = globalElements.input;
const multitoolOutputs = globalElements.output;
...
function acquirement() {
    ...
    const srloc = (() => {
        ...
        if (preSrloc.includes('Space') || srName)
            return preSrloc;
...
function showSizeDropdown() {
    ...
    const smgOption = sizeInput.querySelector('option[value="SMG"]');
    if (smgOption)
        smgOption.style.display = type === 'Experimental' ? 'none' : '';
    if (type === 'Experimental' && size === 'SMG')
        sizeInput.value = 'Pistol';
