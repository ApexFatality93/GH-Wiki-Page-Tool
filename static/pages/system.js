...
const systemPageData = pageData;
const systemInputs = globalElements.input;
const systemOutputs = globalElements.output;
...
const systemPlanetInputs = () => {
    ...
    if (isNaN(bodies) || !inputTarget)
        return;
    const planetInputTarget = inputTarget;
    ...
    function addPlanet(i) {
        ...
        const biomeInput = document.getElementById(`biome_input${i}`);
        if (biomeInput)
            biomeLinks(biomeInput);
        addAllTooltips();
    }
};
...
function removeResource(resourceID) {
    ...
    const addButton = document.getElementById(`addResourceButton${sectionTarget}`);
    if (addButton)
...
function moonSwitch(element) {
    ...
    if (planetClassOutput)
        planetClassOutput.innerText = planetClass;
...
function expandDescriptor(element, planetClass = null) {
    const i = extractNumber(element.id);
    if (!planetClass) {
        const moonInput = document.getElementById('moon_input' + i);
...
function merchantUpgrades(group = null) {
    ...
    function getCheckedBoxes(group) {
        ...
        if (!groupOutput)
            return;
        const wrapper = groupOutput.closest('#scrapDealer');
        if (!wrapper)
            return;
...
const systemAddResource = (element = null) => {
...
systemWindow.addResource = systemAddResource;
function biomeLinks(element) {
    ...
    for (const key in linkedBiomes) {
        const outputElement = globalElements.output[key];
        if (outputElement)
            outputElement.innerText = linkedBiomes[key];
    }
}
...
function expectedHubTagSentence() {
    ...
    if (systemName.includes(expected))
        return;
...
function spaceStationSection() {
    ...
    const faction = (() => {
        ...
        if (input === 'Uncharted')
            return 'uncharted';
        if (input.includes('Abandoned'))
            return 'abandoned';
        if (systemPageData.wealth.includes('Black Market'))
            return 'pirate';
        return 'normal';
    })();
    ...
    if (!notes[faction])
        return;
    const noteSection = document.querySelector('[data-station="note"]');
    if (noteSection)
        noteSection.innerText = notes[faction];
...
function autoPirate(element) {
    const value = element.value;
    if (!value.includes('Black Market') && !value.includes('Pirate Controlled'))
        return;
