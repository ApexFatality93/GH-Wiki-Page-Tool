"use strict";
const planetMoonWindow = window;
const planetMoonPageData = pageData;
const planetMoonInputs = globalElements.input;
const planetMoonOutputs = globalElements.output;
const planetMoonLogicStartupFunctions = () => {
    celestialStartupFunctions();
    autoInfested();
    if (typeof planetStartupFunctions === 'function')
        planetStartupFunctions();
};
...
const planetMoonAddResource = (element = planetMoonInputs.resourceInputs.querySelector('button')) => {
...
planetMoonWindow.addResource = planetMoonAddResource;
function enableResourceAdd() {
    const addButton = planetMoonInputs.resourceInputs.querySelector('button');
    if (addButton)
...
function sentinelSentence() {
    ...
    const sentLevel = (() => {
        for (const level in sentinels) {
            if (sentinels[level].includes(sentDescriptor))
                return level;
        }
        return '';
    })();
...
function addFauna(element) {
    ...
    planetMoonGenusDropdown(globalElements.input[`faunaEcosystemInput${i}`]);
}
...
function postProcessSection(element, sectionType, i) {
    ...
    for (const output of outputs) {
        if (output.id)
            sectionElements.output[output.id] = output.id;
    }
...
function addGenus(element = null) {
    ...
    for (const key in linkedGenera) {
        ...
        if (outputElement)
            outputElement.innerText = linkedGenera[key];
    }
}
...
const planetMoonGenusDropdown = (element) => {
