"use strict";
const celestialPageData = pageData;
const celestialInputs = globalElements.input;
const celestialOutputs = globalElements.output;
...
function autoInfested(element = celestialInputs.descriptionInput) {
    ...
    if (celestialPageData.pageType === 'System')
        return isInfested;
    ...
    const section = (() => {
        for (const biome in data) {
            for (const list in data[biome]) {
                if (data[biome][list]?.includes?.(descriptor.trim()))
                    return list;
            }
        }
    })();
