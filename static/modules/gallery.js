"use strict";
const galleryPageData = pageData;
const galleryInputs = globalElements.input;
const galleryOutputs = globalElements.output;
(() => {
...
function galleryUpload() {
    ...
    for (let fileIndex = 0; fileIndex < files.length; fileIndex++) {
        ...
        document.getElementById(inputId)?.addEventListener('input', function () {
            galleryInput(this, this.dataset.galleryCode ?? '');
        });
        if (typeof generateGalleryArray == 'function')
    ...
    if (typeof galleryExplanationExternal == 'function') {
        explanation('Gallery', `${galleryExplanationExternal()}
...
function mobileMoveItem(element, codeId, direction) {
    ...
    if (!galleryItem || !galleryCodeItem)
        return;
...
galleryOutputs.galleryItems?.addEventListener('click', (event) => {
    const target = event.target;
    const removeTrigger = target?.closest('[data-gallery-remove]');
    if (removeTrigger) {
        rmGallery(removeTrigger, removeTrigger.dataset.galleryRemove ?? '');
        return;
    }
    const moveTrigger = target?.closest('[data-gallery-direction]');
    if (!moveTrigger)
        return;
