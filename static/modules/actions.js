"use strict";
const actionsPageData = pageData;
const actionsOutputs = globalElements.output;
(() => {
    const actions = `
...
<button class="button is-warning" id="reset" type="reset">Reset inputs</button>`;
    ...
    copyButton?.addEventListener('click', () => {
        copyCode(copyButton, 'fullArticle');
    });
    ...
    if (!urlParams.has('debug') || !skipCheckElement)
        return;
...
function reset() {
    ...
    if (typeof resetExternal === 'function')
        resetExternal();
    showAll();
}
